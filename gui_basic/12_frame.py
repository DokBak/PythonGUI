...
# 메뉴 프레임
frame_burger = Frame(root, relief="solid", bd=1) # relief 테두리 모양, bd 외곽선두께 
frame_burger.pack(side = "left",  expand= True)
...
# 11_메시지박스 설정 #
def info(): # 이미지 : i의 원 이미지(파랑)
    response = msgbox.showinfo("알림", "정상적으로 예매 완료되었습니다.")
    print(response)
def warn(): # 이미지 : !의 삼각형 이미지(노랑)
    msgbox.showwarning("경고","해당 좌석은 매진되었습니다.")
def error(): # 이미지 : X의 원 이미지(빨강)
    msgbox.showerror("에러","결재 오류가 발생하였습니다.")
def okcancel(): # 이미지 : ?의 원 이미지(파랑)
    msgbox.askokcancel("확인 / 취소","해당 좌석은 유아 동반석입니다. 예매하시겠습니까?")
def retrycancel(): # 이미지 : !의 삼각형 이미지(노랑)
    ...
    if response == 1: # 네, ok
        ...
        print("아니오")
def yesno(): # 이미지 : ?의 원 이미지(파랑)
    msgbox.askyesno("예 / 아니오","해당 좌석은 역방향입니다. 예매하시겠습니까?")

def yesnocancel():# 이미지 : ?의 원 이미지(파랑)
