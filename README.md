...
2021년 10월 3일
    GUI 프로그램 이미지 합치기프로그램(5_apply_options.py)
        1. 각각의 옵션 값을 받아 각 옵션에 해당하는 값을 설정
        2. 가로 길이 옵션 
            원본 유지
                이미지사이즈변수 = 각 이미지 사이즈를 그대로 적용시켜준다.
            길이 변경
                이미지사이즈변수 = 가로길이는 설정값, 세로길이는 방정식으로 계산한 값으로 설정
        3. 간격 옵션
            스케치북을 생성시 (이미지 갯수-1) * 간격 만큼 세로의 길이를 증가 
            y_offset도 설정한 간격만큼 추가
        4. 포멧 옵션
            확장자까지 고정으로 작성했던 부분을 확장자명만 변수로 변경
    GUI 프로그램 이미지 합치기프로그램(5_apply_options.py)
        1. 저장 경로 버그
            저장경로를 지정 후 취소를 하면 기존 저장경로가 사라지는 버그가 존재 
